console.info("this is imdb-downloader extension");

const pageRefs = document.querySelectorAll("#right > span");
let imageRefs = [];

for (let page = 0; page < pageRefs.length; page++) {
  pageRefs[page]

  const imageRefs2 = Array.from(document.querySelectorAll("#media_index_thumbnail_grid > a"))
  imageRefs = imageRefs.concat(imageRefs2.map((x) => x.href));
  // for (var i = 0, len = imageRefs.length; i < len; i++) {
  //   const ref = imageRefs[i].href;
  //   console.info(ref);
  // }
}
...
(async () => {
  ...
  //const { tip } = await sendMessage({ greeting: 'tip' });
  chrome.runtime.sendMessage({type: 'download-images', pageRefs: pageRefs, imageRefs: imageRefs}, (response) => {
